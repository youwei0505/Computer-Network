...
void printPacketINFO(Packet p)
{
	...
	cout << "(srcPort = " << p.Header.srcPort << ", desPort = " << p.Header.desPort << ") " ;
}
...
{ 
	...
	int fileSize; 
    ofstream fileOut1;
	fileOut1.open("2.mp4", ios::out | ios::binary);
	...
	sendto(client_socket, &packet_sender , sizeof(Packet), 0,(struct sockaddr*)&server_addr, server_addr_size);
   	...
   	while(fileSize)
	{
	recvfrom(client_socket, &fff , sizeof(fff), 0, (struct sockaddr*)&server_addr, &server_addr_size); 
	//cout << fff[0];
	fileOut1.put(fff[0]) ; 
	packet_sender.set("ACK", client_port, server_port, seq, ack, BUFFER_SIZE, NULL);
	sendto(client_socket, &packet_sender , sizeof(Packet), 0,(struct sockaddr*)&server_addr, server_addr_size);
